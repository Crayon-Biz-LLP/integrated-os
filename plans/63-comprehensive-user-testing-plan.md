...
| ID | Scenario | Steps | Expected Outcome | Pass/Fail |
|----|----------|-------|------------------|-----------|
| P1 | **Create task with org context** | Send: "Review pricing for Solvstrat client — due tomorrow 2pm" | 1. Org resolved → "SOLVSTRAT" via Action Planner<br>2. Task created with `organization_id` (no `project_id`)<br>3. Task node created in `graph_nodes` (type='task')<br>4. BELONGS_TO edge to Solvstrat org or project-type node<br>5. Google Calendar: 2pm with `⚡ ACTION:` prefix<br>6. Google Tasks entry with title | ☐ |
| P2 | **Create task with org (no project-type node)** | Send: "Handle the Ashraya compliance filing — due next Monday" | 1. Org resolved to Ashraya (no project-type node needed)<br>2. Task created with `organization_id`<br>3. Proper routing in Pulse briefings | ☐ |
