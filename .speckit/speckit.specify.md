...
- **DB-Backed Clarification State**: In-memory `pending_graph_clarifications` dict (handler.py) and `active_sessions` dict (graph.py) replaced with `pending_graph_clarifications` table — survives Vercel cold restarts. `core/lib/clarification_state.py` with `get_active_clarification()`, `set_clarification()`, `resolve_clarification()`.
