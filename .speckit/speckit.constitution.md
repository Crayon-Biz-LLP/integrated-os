...
6. **Documentation is updated immediately** — the relevant `product-summary/XX-*.md` file(s) and `.speckit/` artifacts (plan, tasks, spec) must reflect the change. If the change introduces new behavior, a new `product-summary/` file is created and listed in `README.md`. Documentation debt is code debt.
