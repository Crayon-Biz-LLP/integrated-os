...
def process_message_background(payload: dict):
    ...
    payload: {"text": str, "session_id": str | None, "uid": str | None}
    ...
    resolve_channel_tenant() = the first active user (tenant #1).
    """
    import asyncio
    ...
    uid = payload.get("uid")
    if not text:
        print("[process_message_background] Missing text — aborting")
        return
    if uid:
        from core.services.db import tenant_scope
        with tenant_scope(uid):
            asyncio.run(_run_web_message_pipeline(text, session_id))
    else:
        asyncio.run(_run_web_message_pipeline(text, session_id))
