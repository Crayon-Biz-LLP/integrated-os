...
core/skills/whatsapp_ingest.py::process_whatsapp_message()
  ...
  3. Insert into whatsapp_messages table
...
```
The Decision Pulse is not part of the main AI briefing. It runs independently via `process_decision_pulse()` — fetches pending items from `whatsapp_messages`, `email_pending_tasks`, and `call_pending_items`, formats with shortcodes, sends to Telegram. No AI generation, ~2s runtime.
...
Prefixed shortcodes (`w{id}`) route directly to `whatsapp_messages` via `core/webhook/whatsapp.py`. Unprefixed shortcodes (`{id}`) fall back through email → call → WhatsApp → practice dismissal.
...
1. **Separate table** (`whatsapp_messages`) rather than reusing `emails` — keeps WhatsApp independent from the email pipeline
