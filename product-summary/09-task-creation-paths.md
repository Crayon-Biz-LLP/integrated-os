...
- **Recurring Tasks (`UNTIL` Support):** The `TASK` extraction models support infinite and finite recurrences. If the user says "every Wednesday until August 31", the system generates an iCalendar RRULE (e.g., `RRULE:FREQ=WEEKLY;UNTIL=20260831T000000Z`) and pushes it directly to Google Calendar. Recurring tasks handle status changes as follows:
  ...
  - **UNTIL boundary exhaustion** (June 2026): When the user marks a recurring task `done` but the RRULE `UNTIL` date is already past (no future instances remain), `skip_recurring_instance` returns `"No upcoming instances found"`. `update_task_status` treats this signal as a **permanent close** — the master task is marked `done` (not `todo`), ending the series cleanly. Previously, the task would loop back to `todo` forever even after the series expired.
  - **Auto-expiry:** The pulse engine's `_auto_expire_recurring_tasks()` checks the RRULE `UNTIL` date (supports both `YYYYMMDDTHHMMSSZ` and `YYYYMMDD` formats) and `COUNT`-based recurrences. Note: auto-expiry marks tasks as `cancelled`; user-initiated `done` on an exhausted series marks as `done` via the UNTIL boundary path above.
  - **`recurrence="none"` guard** (June 2026): Non-recurring tasks store `recurrence="none"` (a string). Because `"none"` is truthy in Python, they were incorrectly entering the recurring skip path. The guard in `update_task_status` is now `td.get('recurrence') not in ['none', '']` to treat them as non-recurring.
