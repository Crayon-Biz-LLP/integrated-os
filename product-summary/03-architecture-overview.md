...
The brain of the system. A 1,757-line scheduled intelligence cycle that fetches all pending data, classifies it, runs 5 parallel AI agents, generates a structured briefing, writes changes back to the database, syncs with Google, and delivers the briefing via Telegram.
...
### Agents (`core/agents/`)
Autonomous workers: research agent (Jina AI web search), janitor (pipeline health), cleanup (orphan records), quick process (inline raw dump processing).
...
Scheduled Pulse (via GitHub Actions)
    ...
    → pulse_cli.py → engine.py:
        ...
        → Fetch raw_dumps + tasks + projects + people + missions
        ...
        → Write phase (projects, people, tasks, completions, missions)
