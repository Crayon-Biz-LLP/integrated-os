...
|-------------|---------------|---------|
| Image (whiteboard photo) | Tasks extracted from handwritten text | "Follow up on Qhord Q3 pricing" |
| Voice memo | Transcription → task extraction | "Call Sunju about the church event" → task created |
| PDF/DOCX | Content parsed → tasks extracted | "Meeting notes → 3 action items → 3 tasks created |
