...
### Entry Point
A single serverless function at `api/index.py` handles all HTTP traffic via Vercel `rewrites`. 13 API routes:
...
| `/api/graph-edge-action` | POST | X-API-Key | Approve/edit/reject pending graph edge |
...
```
api/index.py                      — FastAPI app, 12 routes, auth middleware (~494 lines)
core/
...
│   ├── engine.py                 — Main briefing cycle (~1,492 lines)
...
│   ├── llm.py                    — LLM calls with fallback chain (ToolRegistry moved to tools.py)
│   ├── tools.py                  — ToolRegistry: tool definitions for agent loop
...
└── lib/                          — Utilities
    ...
    ├── rate_limiter.py           — Sliding window distributed rate limiter (Redis-backed)
    ├── redis_cache.py            — Distributed Upstash Redis cache (get/set/delete, rate-limit check)
...
| Resources | Library grid, cluster groups, category filters |
