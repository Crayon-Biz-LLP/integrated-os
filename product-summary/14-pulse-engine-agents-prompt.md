...
1. **Cross-domain keyword bridges**: Finds words >4 chars appearing in 2+ task titles from different org_tags
2. **People-in-resources**: Finds person names appearing in resource descriptions across the knowledge base
3. **Temporal serendipity**: Notes recent resource additions that may have hidden connections to active tasks
...
- NEVER use: momentum, focus, gentle, reflection, push, strategic, SITREP, optimal,
  mission, ready for your review, Operational, Vanguard, Battlefield, Chief of Staff
...
### Mission Insert (engine.py:1507-1519)
For AI-generated `new_missions` (triggered when 3+ items suggest a cohesive new goal). After insert, historical resources are backfilled against the new mission at ≥0.80 confidence.
