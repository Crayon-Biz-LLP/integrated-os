...
### Decision Pulse Classification Guard

Email items in the Decision Pulse are filtered to only show `actionable` classifications — `fyi` and `ignored` emails are excluded. This filter was missing after the unified `messages` table merge (WhatsApp and Teams channels had it correctly). 158 false-positive `fyi`/`ignored` emails were backfilled to `danny_decision = 'skipped'`.

### "Untitled" Decision Pulse Fix

The classification prompt permits `suggested_task: null` for actionable emails where the specific action is "too vague to state explicitly." When this happens, the ingest code (`outlook_ingest.py:434`) skips setting `suggested_title`, leaving it NULL in the `messages` table.

The Decision Pulse (`engine.py:283`) now falls back to the email's `subject` when `suggested_title` is NULL, avoiding unhelpful "Untitled" rows in the pulse output:

```python
title = row.get('suggested_title') or row.get('subject') or 'Untitled'
```

A one-time backfill was also applied:
```sql
UPDATE messages
SET suggested_title = subject
WHERE channel = 'email'
