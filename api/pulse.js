...
export default async function handler(req, res) {
    try {
        ...
        const { data: active_tasks } = await supabase.from('tasks').select('*').neq('status', 'done');
        ...
        if (expiryMatch && now > new Date(expiryMatch[1])) system_context = "CRITICAL: Season Context EXPIRED.";
    ...
    `;
        ...
        const model = genAI.getGenerativeModel({
            model: "gemini-1.5-pro", // Switched to Pro since GitHub Actions handles the time
        ...
        // AUTO-EXPANSION (People)
        if (aiData.new_people?.length) {
            await supabase.from('people').insert(aiData.new_people);
        }
        ...
        if (aiData.new_tasks?.length) {
            const taskInserts = aiData.new_tasks.map(t => {
                const matchedProject = projects.find(p => p.name.toLowerCase().includes(t.project_name?.toLowerCase())) || projects[0];
                return {
                    title: t.title,
                    project_id: matchedProject.id,
                    priority: t.priority?.toLowerCase() || 'important',
                    estimated_minutes: t.est_min || 15
                };
            });
        ...
        if (aiData.logs?.length) {
            await supabase.from('logs').insert(aiData.logs);
        }
        ...
        if (process.env.TELEGRAM_CHAT_ID && aiData.briefing) {
            await fetch(`https://api.telegram.org/bot${process.env.TELEGRAM_BOT_TOKEN}/sendMessage`, {
                ...
                body: JSON.stringify({ chat_id: process.env.TELEGRAM_CHAT_ID, text: aiData.briefing, parse_mode: 'Markdown' })
            });
        }
        ...
        console.error('Pulse Failure:', error);
