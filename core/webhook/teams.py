...
async def process_teams_pending_decision(pending_id: int, decision: str, supabase_client=None) -> dict:
    """Process approve/reject for a Teams pending message.

    For 'approve': writes to raw_dumps (Pulse classifies/routes on next run).
    For 'reject': sets danny_decision='rejected'.

    Args:
        pending_id: ID in messages table with channel='teams'.
    ...
    if not row_res.data:
        ...
        return {
            ...
            "message": f"No Teams message found matching [{pending_id}]."
        }

    row = row_res.data
    title = row.get('suggested_title') or row.get('body', '')[:60]
    ...
    if decision == 'approve':
        ...
        print(f"Staged to raw_dumps via Teams approval: {title}")
